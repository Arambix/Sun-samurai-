...
dark_blue = (80,100,179)
background_colour = dark_blue
white = (255,255,255)
light_blue1 = (150,160, 250)
board_colour = light_blue1
red = (200,100,100)
red2 = (150,60,80)
black = (0,0,0)
green = (165,210,185)
...
playtext = font.render('Play', True, light_blue1, white)
tutorialtext = font.render('Tutorial', True, light_blue1, white)
optionstext = font.render('Options', True, light_blue1, white)
quittext = font.render('Quit', True, light_blue1, white)
whitewinstext = font.render('White Wins!', True, black, white)
blackwinstext = font.render('Black Wins!', True, white, black)
playtextsurface = playtext.get_rect()
stop_event = threading.Event() # creating a method of communication between main and other threads
...
global checking
seconds = 0
black_pawn_image = pygame.image.load(f"img/Chess Game/black-pawn.png")
black_pawn_image = pygame.transform.scale(black_pawn_image, (100, 100))

white_pawn_image = pygame.image.load(f"img/Chess Game/white-pawn.png")
white_pawn_image = pygame.transform.scale(white_pawn_image, (100, 100))

# Knights
black_knight_image = pygame.image.load(f"img/Chess Game/black-knight.png")
black_knight_image = pygame.transform.scale(black_knight_image, (100, 100))

white_knight_image = pygame.image.load(f"img/Chess Game/white-knight.png")
white_knight_image = pygame.transform.scale(white_knight_image, (100, 100))

# Bishops
black_bishop_image = pygame.image.load(f"img/Chess Game/black-bishop.png")
black_bishop_image = pygame.transform.scale(black_bishop_image, (100, 100))

white_bishop_image = pygame.image.load(f"img/Chess Game/white-bishop.png")
white_bishop_image = pygame.transform.scale(white_bishop_image, (100, 100))

# Rooks
black_rook_image = pygame.image.load(f"img/Chess Game/black-rook.png")
black_rook_image = pygame.transform.scale(black_rook_image, (100, 100))

white_rook_image = pygame.image.load(f"img/Chess Game/white-rook.png")
white_rook_image = pygame.transform.scale(white_rook_image, (100, 100))

# Queens
black_queen_image = pygame.image.load(f"img/Chess Game/black-queen.png")
black_queen_image = pygame.transform.scale(black_queen_image, (100, 100))

white_queen_image = pygame.image.load(f"img/Chess Game/white-queen.png")
white_queen_image = pygame.transform.scale(white_queen_image, (100, 100))

# Kings
black_king_image = pygame.image.load(f"img/Chess Game/black-king.png")
black_king_image = pygame.transform.scale(black_king_image, (100, 100))

white_king_image = pygame.image.load(f"img/Chess Game/white-king.png")
...
class Board(): # board class will contain each piece and functions associated with the playing of the game
    def __init__(self):
        ...
        self.seconds = 0
        self.timetext = font.render(str(self.seconds), True, white, black)
    def timer(self):
    ...
    def change_turn(self):
        ...
        if self.turn == "black":
            self.turn_number +=1
            return "white"
    def try_combinations(self,newboard):
        allmoves = []
        for pieces in chess_pieces[board.turn]:
            for piece in chess_pieces[board.turn][pieces]:
                for column in range(8):
                    for row in range(8):
                        if piece.determine_move([piece,piece.posx,piece.posy],column,row, newboard):
                            allmoves.append([piece, column, row])
                            #print([piece, row, column])
        return allmoves
    def test_board(self,piece,column,row, inputboard):
        newboard = copy.copy(inputboard)

        board15 = piece.move([piece,piece.posx,piece.posy],column,row,newboard)[1]

        return board15
    def try_check(self,newboardinput,checkinginput):
        newboard = copy.copy(newboardinput)

        if self.turn == "black":
            if checkinginput.determine_move([checkinginput, checkinginput.posx,checkinginput.posy], chess_pieces["white"]["king"][0].posx, chess_pieces["white"]["king"][0].posy, newboard) == True:
                return True
        if self.turn == "white":
            if checkinginput.determine_move([checkinginput, checkinginput.posx,checkinginput.posy], chess_pieces["black"]["king"][0].posx, chess_pieces["black"]["king"][0].posy, newboard) == True:
                return True
    def check_defence(self,inputboard):
        newboard = copy.copy(inputboard)
        allmoves = board.try_combinations(newboard)
        #print(allmoves)
        legalmoves = []
        for i in range(len(allmoves)):
            ...
            x = allmoves[i][0].posx
            y = allmoves[i][0].posy
            newboard = self.test_board(allmoves[i][0], allmoves[i][1], allmoves[i][2], newboard)
            self.turn = self.change_turn()
            allmoves[i][0].posx = allmoves[i][1]
            allmoves[i][0].posy = allmoves[i][2]
            
            for pieces in chess_pieces[board.turn]:
                for piece in chess_pieces[board.turn][pieces]:
                    #print(piece,piece.posx,piece.posy)
                    if (self.try_check(newboard, piece)) == True: 
                        append = False
                        
                    
            if append == True:
                legalmoves.append(allmoves[i])
            allmoves[i][0].posx = x
            allmoves[i][0].posy = y
    ...
    def compare_check_defence(self, inputboard, piece,x,y):
        newboard = copy.copy(inputboard)
        legalmoves = self.check_defence(newboard)
        #print(legalmoves)
        if legalmoves == []:
            enable_checkmate = threading.Thread(target=self.cause_checkmate, args = ())
            enable_checkmate.start()
       
        for i in range(len(legalmoves)):
            if piece.type == legalmoves[i][0].type:
    ...
    def start_game(self): # attribute of the board class created with a method to change it from one to the other, which the game loop uses to test if the board should be displayed
        if self.active == False:
            ...
            self.active = False
    def show_board(self, colour): # method which will draw the board when self.active is true
        for column in range(8):
            for row in range(8):
                if (column + row)%2 == 0:
                    pygame.draw.rect(surface,colour,[100+100*column,100*row,100,100])
...
class Piece(): # piece class used as a class from which each different piece inherits 
    def __init__(self, colour,x,y): 
        ...
        self.posx = x
        self.posy = y 
    def move(self,selected, x, y, inputboard): # parameters of selected and x,y are given so that we have the piece clicked previously, and the square that has been clicked most recently
        #print(selected[1],selected[2])
        newboard = []
        for row in inputboard: # the move function is intended to output what the board would be when a move is made. A copy needs to be made to stop the real board from being changed
            newrow = copy.copy(row) # the copy.copy func is used because it creates a copy of the overall structure. It was required for me to copy each row because i need the objects
            newboard.append(newrow) # to still be references, but if i copy the whole 2d array, the inner lists would be references too, which would change the board inadvertantly
        if (selected[1],selected[2]) == (x,y):# makes sure the piece cannot capture itself
            return [False, newboard, x, y]
        if selected[0] != None:
            #print(selected[0].colour)
            if selected[0].colour == board.turn: # checking if it is right players turn
                if newboard[y][x] != None:
                    if newboard[7][7] !=None:
                        if newboard[y][x].type == "Rook" and newboard[y][x].colour == newboard[self.posy][self.posx].colour and newboard[y][x].turns == 0 and self.turns == 0 and x == 7 and y == 7: # castling move
                            if newboard[7][5] == None and newboard[7][6] == None:
                                newboard[7][5] = newboard[7][4]
                                newboard[7][6] = newboard[7][7]
                                newboard[7][4] = None
                                newboard[7][7] = None
                                return [True,newboard,x,y]
                ...
                if x == selected[1]+1 or x == selected[1]-1: # pawns can move diagonally. These are the columns either side of pawn being checked
                    if selected[2] == y +1: # can only move 1 space
                        if newboard[y][x] != None: # can only move diagonally when capturing
                            if newboard[y][x].colour != newboard[self.posy][self.posx].colour and [x,y] != [selected[0],selected[1]]:
                                return True
            if board.turn == "black":
                if x == selected[1]:
                    if y == selected[2]+1:
                ...
                if x == selected[1]+1 or x == selected[1]-1:
                    if selected[2] == y -1:
                        if newboard[y][x] != None and newboard[self.posy][self.posx] != None:
                            if newboard[y][x].colour != newboard[self.posy][self.posx].colour and [x,y] != [selected[0],selected[1]]:
                                return True
    ...
    def button_check(self, mousex, mousey):
        if mousey > 40 and mousey < 760:
            if mousex > 100 and mousex < 270:
                self.active = False
                board.active = True
                timer = threading.Thread(target=board.timer, args=())
                timer.start()
...
print(chess_pieces["white"]["queen"])
...
while Running:
    ...
    for event in pygame.event.get(): 
        if event.type == pygame.QUIT: 
            stop_event.set() # must end all threads to avoid a crash when the game ends
            Running = False
            ...
            if board.active == True:
                if selected[0] != None: # making sure we dont use a method on None which will cause a syntax error
                    ...
                    if True:
                        #print("blackcheck")       
                        #print(checking)                 
                        if board.compare_check_defence(board.board,selected[0], (mousex-100)//100,mousey//100):
                            #print("blackking")
                            pass
                        else:
                            #print("no")       
                    ...
                    if True:   
                        ...
                        if board.compare_check_defence(board.board,selected[0], (mousex-100)//100,mousey//100):
                            ...
                            #print("no")  
                            legal = False         
                        #print(board.try_combinations(board.board))    
                    if legal == True:                    
                        if selected[0].determine_move(selected,(mousex-100)//100,mousey//100, board.board) == True:
                            savedmove = selected[0].move(selected,(mousex-100)//100,mousey//100, board.board)
                            if savedmove[0] == True:
                                selected[0].turns += 1 
                                board.board = savedmove[1]
                                selected[0].posx = savedmove[2]
                                selected[0].posy = savedmove[3]
                                #print(chess_pieces["white"]["queen"])
                                #print(board.board[7][3])
                                #print(selected[0])
                                if board.try_check(board.board,selected[0]) == True:
                                    if board.turn == "white":
                                        ...
                                        checking = selected[0]
                                        print(checking,checking.posx,checking.posy)
                                    if board.turn == "black":
                                        chess_pieces["white"]["king"][0].in_check = True
                                        checking = selected[0]
                                        print(checking,checking.posx,checking.posy)
                                else:
                                    chess_pieces["black"]["king"][0].in_check = False
                                    chess_pieces["white"]["king"][0].in_check = False
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_c:
                if board_colour == light_blue1:
                    board_colour = green        
                    background_colour = green2          
                elif board_colour == green:
                    board_colour = red
                    background_colour = red2
                elif board_colour == red:
                    board_colour = yellow1
                    background_colour = yellow2
                elif board_colour == yellow1:
                    board_colour = light_blue1
                    background_colour = dark_blue
    ...
    if menu.active == True:
        menu.show_menu()
    if board.active == True:
        board.show_board(board_colour)
        playtextsurface.center = (50, 400) 
        surface.blit(board.timetext, playtextsurface)

    if board.checkmate == True:
        if board.turn == "white":            
            playtextsurface.center = (400, 300) 
            surface.blit(blackwinstext, playtextsurface)
        else:
            playtextsurface.center = (400, 300) 
            surface.blit(whitewinstext, playtextsurface)
